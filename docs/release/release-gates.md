...
| Public source readiness (RP-1) | READY AFTER OWNER ACTIONS | MPL-2.0 source license, governance, clean Git-tracked export, source-only CI, and snapshot scanning are in place. Create the public repository with fresh history only after configuring the exact GitHub noreply identity and confirming MFA. |
...
| SignPath Foundation eligibility | ACTION REQUIRED | Public repository, MFA confirmation, SignPath application, trusted GitHub origin, manual approval policy, and final artifact configuration are not yet configured. |
